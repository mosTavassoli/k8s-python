...
def calculate_data():
    ...
    sem = round(s.sem(data), 5)
    conf_interval = s.t.interval(confidence=(
        0.95), df=len(data)-1, loc=(mean), scale=(sem))
    ...
    ax1.hist(data, bins=20, color='orange')
    plt.axvline(mean, color='red', label='Mean')
    plt.axvline(median, color='green', label='Median')
    # plt.axvline(std,color='blue',label='std')
    plt.axvline(x=conf_interval[0], linestyle='--',
                linewidth=2, c='crimson', label='lower 95% CI bound')
    plt.axvline(x=conf_interval[1], linestyle='--',
                linewidth=2, c='purple', label='upper 95% CI bound')
    plt.axvspan(conf_interval[0], conf_interval[1],
...
#    plt.savefig('mean_std.png')
    ...
    with open("results.txt", "a") as f:
        ...
        f.write("Confidence Interval: " + str(conf_interval) + "\n")
...
# kubectl autoscale deployment nginx-deployment --cpu-percent=25 --min=1 --max=1
